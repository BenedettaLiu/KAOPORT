...
export default function HomeScreen() {
  ...
  const { data: snapshot, isFetching, isLoading, refetch } = trpc.ships.snapshot.useQuery(undefined, {
    staleTime: 60_000,
  ...
  const filteredShips = useMemo(() => filterShips(records, query, activeFilter, { name: nameFilter, originPort: originPortFilter }), [activeFilter, nameFilter, originPortFilter, query, records]);
  ...
  const handleRefresh = useCallback(async () => {
    const result = await refreshShips.mutateAsync();
    await setFavoriteRecords(await getFavoriteRecords(result.snapshot.ships));
    await refetch();
    ...
    <ScreenContainer containerClassName="bg-background" edges={["top", "left", "right"]}>
      <FlatList contentContainerStyle={styles.listContent} data={filteredShips} keyExtractor={(item) => item.id} keyboardShouldPersistTaps="handled" refreshControl={<RefreshControl colors={["#0B4F71"]} onRefresh={handleRefresh} refreshing={isFetching && !isLoading} tintColor="#0B4F71" title="正在同步官方船期…" titleColor="#506773" />}
        ...
        ListHeaderComponent={<View><View style={styles.header}><View style={styles.headerCopy}><Text style={styles.eyebrow}>KAOHSIUNG PORT</Text><Text style={styles.heading}>船舶動態</Text><Text style={styles.subheading}>高雄港官方即時船期</Text></View><View style={[styles.sourceBadge, snapshot?.source === "official" && styles.officialBadge]}><Text style={[styles.sourceBadgeText, snapshot?.source === "official" && styles.officialBadgeText]}>{snapshot?.source === "official" ? "官方資料" : snapshot?.source === "cached" ? "快取資料" : "同步中"}</Text></View></View>
          ...
          <Pressable accessibilityRole="button" accessibilityState={{ expanded: showAdvancedFilters }} onPress={() => setShowAdvancedFilters((visible) => !visible)} style={({ pressed }) => [styles.advancedToggle, pressed && styles.buttonPressed]}><MaterialIcons color="#176B85" name="tune" size={18} /><Text style={styles.advancedToggleText}>進階篩選：來源港與船名</Text><MaterialIcons color="#176B85" name={showAdvancedFilters ? "expand-less" : "expand-more"} size={21} /></Pressable>
          {showAdvancedFilters ? <View style={styles.advancedPanel}><Text style={styles.advancedLabel}>來源港</Text><TextInput accessibilityLabel="依來源港篩選" clearButtonMode="while-editing" onChangeText={setOriginPortFilter} placeholder="例如：新加坡、台中港" placeholderTextColor="#7C8D98" returnKeyType="done" style={styles.advancedInput} value={originPortFilter} /><Text style={styles.advancedLabel}>船名</Text><TextInput accessibilityLabel="依船名篩選" autoCapitalize="characters" clearButtonMode="while-editing" onChangeText={setNameFilter} placeholder="例如：EVER、MAERSK" placeholderTextColor="#7C8D98" returnKeyType="done" style={styles.advancedInput} value={nameFilter} />{nameFilter || originPortFilter ? <Pressable onPress={() => { setNameFilter(""); setOriginPortFilter(""); }} style={({ pressed }) => [styles.advancedClear, pressed && styles.buttonPressed]}><Text style={styles.advancedClearText}>清除進階條件</Text></Pressable> : null}</View> : null}
