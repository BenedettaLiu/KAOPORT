    ...
    <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
      ...
      <View style={styles.section}>
        ...
        <View collapsable={false} ref={specificationRef} style={styles.specCaptureCard}>
          <View style={styles.specGrid}>
            <SpecTile icon="directions-boat" label="英文船名" value={ship.name} />
            <SpecTile icon="translate" label="中文船名" value={ship.chineseName ?? "尚未提供"} />
            <SpecTile icon="numbers" label="IMO" value={ship.imo} />
            <SpecTile icon="pin" label="MMSI" value={ship.mmsi ?? "尚未提供"} />
            <SpecTile icon="settings-input-antenna" label="呼號" value={ship.callSign ?? "尚未提供"} />
            <SpecTile icon="directions-boat" label="船型" value={ship.vesselType} />
            <SpecTile icon="public" label="船籍" value={ship.flag} />
            <SpecTile icon="straighten" label="船總長度" value={ship.overallLength ?? "尚未提供"} />
      ...
      <View style={styles.section}>
        ...
        <View style={styles.infoCard}>
          ...
          <View style={styles.dashboardLinks}><Pressable accessibilityLabel="開啟高雄港最新動態看板" onPress={() => openOfficialDashboard(OFFICIAL_LATEST_MOVEMENT_URL, "最新動態看板")} style={({ pressed }) => [styles.dashboardLink, pressed && styles.buttonPressed]}><MaterialIcons color="#0B5D7E" name="dynamic-feed" size={19} /><View style={styles.dashboardLinkCopy}><Text style={styles.dashboardLinkTitle}>最新動態看板</Text><Text style={styles.dashboardLinkText}>開啟高雄港船舶動態系統</Text></View><MaterialIcons color="#0B5D7E" name="open-in-new" size={18} /></Pressable><Pressable accessibilityLabel="開啟高雄港最新船期看板" onPress={() => openOfficialDashboard(OFFICIAL_LATEST_SCHEDULE_URL, "最新船期看板")} style={({ pressed }) => [styles.dashboardLink, pressed && styles.buttonPressed]}><MaterialIcons color="#0B5D7E" name="event-note" size={19} /><View style={styles.dashboardLinkCopy}><Text style={styles.dashboardLinkTitle}>最新船期看板</Text><Text style={styles.dashboardLinkText}>開啟高雄港船舶動態系統</Text></View><MaterialIcons color="#0B5D7E" name="open-in-new" size={18} /></Pressable></View>
        </View>
      </View>
...
const styles = StyleSheet.create({
  ...
  specCaptureCard: { backgroundColor: "#F8FBFC", borderRadius: 16, padding: 1 }, specGrid: { flexDirection: "row", flexWrap: "wrap", gap: 10 }, specTile: { backgroundColor: "#FFFFFF", borderColor: "#C8E0E8", borderRadius: 14, borderWidth: 1, minHeight: 93, padding: 12, width: "48.5%" }, specLabel: { color: "#3F6475", fontSize: 11, fontWeight: "800", lineHeight: 16, marginTop: 6 }, specValue: { color: "#284252", fontSize: 13, fontWeight: "800", lineHeight: 19, marginTop: 2 }, specActionRow: { flexDirection: "row", gap: 10, marginTop: 11 }, specAction: { alignItems: "center", backgroundColor: "#EAF6FA", borderColor: "#B7DCE8", borderRadius: 12, borderWidth: 1, flex: 1, flexDirection: "row", justifyContent: "center", minHeight: 43 }, specActionText: { color: "#0B5D7E", fontSize: 13, fontWeight: "800", marginLeft: 6 }, specActionNotice: { color: "#167A54", fontSize: 12, fontWeight: "700", lineHeight: 18, marginTop: 7, textAlign: "center" }, voyageCard: { alignItems: "center", backgroundColor: "#FFFFFF", borderColor: "#DCE6EB", borderRadius: 14, borderWidth: 1, flexDirection: "row", marginTop: 10, minHeight: 48, paddingHorizontal: 13 }, voyageCardLabel: { color: "#3F6475", fontSize: 12, fontWeight: "800", marginLeft: 8 }, voyageCardValue: { color: "#284252", flex: 1, fontSize: 13, fontWeight: "800", textAlign: "right" }, dashboardLinks: { marginHorizontal: -15, paddingHorizontal: 15, paddingTop: 4 }, dashboardLink: { alignItems: "center", borderTopColor: "#D9E8ED", borderTopWidth: 1, flexDirection: "row", minHeight: 61, paddingVertical: 9 }, dashboardLinkCopy: { flex: 1, marginLeft: 9, minWidth: 0 }, dashboardLinkTitle: { color: "#0B5D7E", fontSize: 13, fontWeight: "800", lineHeight: 19 }, dashboardLinkText: { color: "#4A6D7C", fontSize: 11, lineHeight: 16, marginTop: 1 },
